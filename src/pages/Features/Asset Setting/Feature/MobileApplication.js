...
const MobileApplication = () => {
    ...
    const [updatedApplicationName, setupdatedApplicationName] = useState(); // modal to stored edited or updated value
    const [updatedApplicationPlatform, setupdatedApplicationPlatform] = useState(); // modal to stored edited or updated value
                ...
                <>
                    <AvForm>
                        ...
                        <Button
                            ...
                            onClick={() => {
                                tog_standard4();
                                setupdatedApplicationName(productData.ApplicationName)
                                setupdatedApplicationPlatform(productData.ApplicationPlatform)
    ...
    let editApplicationPlatformRef = useRef(null); // ref for severity
    ...
    async function getassetListData(event) {
        const response = await fetch(
            `${process.env.REACT_APP_DEFAULTPATH}getassetlist-mobileApplication`,
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
        ...
        if (data) {
            ...
            setProductData(data.mobileApplicationList);
    ...
    async function editmobileApplicationValue() {
        ...
        swal({
            ...
            .then(async (willDelete) => {
                if (willDelete) {
                    console.log("updatedValue", editApplicationPlatformRef.current)
                    let formData = new FormData();
                    formData.append("apkFIle", updatedValue[0])
                    formData.append("orgID", orgID)
                    formData.append("updatedApplicationName", updatedApplicationName)
                    formData.append("updatedApplicationPlatform", updatedApplicationPlatform)
                    formData.append("updatedValueID", updatedValueID)
                    await axios.post(`${process.env.REACT_APP_DEFAULTPATH}edit-mobile-application-assetlist`, formData).then((res) => {
                        getassetListData()
    ...
    async function deleteElement(id) {
        swal({
            ...
            .then(async (willDelete) => {
                if (willDelete) {
                    console.log("updatedValueID", updatedValueID)
                    await axios.post(`${process.env.REACT_APP_DEFAULTPATH}delete-assetlist-mobileapplication-element`, {
            ...
            <div className="userProfile credentialBreachProfile">
                ...
                <Row>
                    <Breadcrumbs
                        ...
                        breadcrumbItem="Mobile Application "
                        breadcrumbFeature="Mobile Application"
                    />
                </Row>
                        ...
                        <div className="modal-body">
                            <div>
                                ...
                                <Input
                                    type="text"
                                    id="applicationName"
                                    placeholder={updatedApplicationName}
                                    className="mb-3"
                                    onChange={(e) => {
                                        // console.log("val", e.target.value);
                                        setupdatedApplicationName(e.target.value)
                                    }}
                                />
                                ...
                                <Select
                                    ...
                                    placeholder={updatedApplicationPlatform}
                                    className="mb-4"
                                    options={ApplicationPlatformOptions}
                                    ref={editApplicationPlatformRef}
                                    onChange={(e)=>{
                                        setupdatedApplicationPlatform(e.value)
                                    }}
                                />
                            </div>
                            {/* <input type="text" id="appName" name="myfile" onChange={(e) => { setupdatedApplicationName(e.target.value) }}></input> */}
                            <input type="file" id="myfile" name="myfile" accept=".apk,.ipa" onChange={(e) => { setupdatedValue(e.target.files) }}></input>
