...
const SourceCode = () => {
    ...
    // const [updatedProjectName, setupdatedProjectName] = useState(); // modal to stored edited or updated value
    const [updatedProjectName, setupdatedProjectName] = useState(); // modal to stored edited or updated value
    const [updatedProjectLanguage, setupdatedProjectLanguage] = useState(); // modal to stored edited or updated value
                ...
                <>
                    <AvForm>
                        ...
                        <Button
                            ...
                            onClick={() => {
                                tog_standard4();
                                seteditValue(productData.Value)
    ...
    async function getassetListData(event) {
        const response = await fetch(
            `${process.env.REACT_APP_DEFAULTPATH}getassetlist-sourcecode`,
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
        ...
        if (data) {
            ...
            setProductData(data.sourceCodeList);
        ...
        swal({
            ...
            .then(async (willDelete) => {
                if (willDelete) {
                    ...
                    formData.append("updatedProjectLanguage", updatedProjectLanguage);
                    formData.append("updatedValueID", updatedValueID)
                    await axios.post(`${process.env.REACT_APP_DEFAULTPATH}edit-sourcecode-assetlist`, formData).then((res) => {
                        getassetListData()
        ...
        swal({
            ...
            .then(async (willDelete) => {
                if (willDelete) {
                    ...
                    await axios.post(`${process.env.REACT_APP_DEFAULTPATH}delete-assetlist-sourcecode-element`, {
                    ...
                    >
                        ...
                        <div>
                                ...
                                <Input
                                    type="text"
                                    id="applicationName"
                                    placeholder={updatedProjectName}
                                    className="mb-3"
                                    onChange={(e) => {
                                ...
                                <Label htmlFor="applicationLanguage">
                                    Project Language
                                </Label>
                                <Select
                                    id="applicationLanguage"
                                    placeholder={updatedProjectLanguage}
                                    className="mb-4"
                                    options={ProjectLanguageOptions}
                                    ref={projectLanguageRef}
                                    onChange={(e)=>{
                                        // console.log("data chnages", e.value)
                                        setupdatedProjectLanguage(e.value)
                                    }}
                                />
                            </div>
                            {/* <input type="text" id="appName" name="myfile" onChange={(e) => { setupdatedProjectName(e.target.value) }}></input> */}
                            <Input type="file" id="myfile" name="myfile" accept=".rar" onChange={(e) => { setupdatedValue(e.target.files) }}></Input>
