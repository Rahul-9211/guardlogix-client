  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (cellContent, productData) => (
        <>
          {(() => {
            if (productData.Domain === null) {
              return <>-</>;
            } else {
              return  <>{productData.Domain}</>;
            }
          })()}
        </>
      ),
...
useEffect(async () => {
  ...
  async function getReverseDNSData(event) {
    ...
    const data = await response.json();
    if (data) {
      // console.log("response reverseDNS ", data);
  ...
  return (
    <React.Fragment>
      <div className="userProfile credentialBreachProfile">
        <MetaTags>
          <title>Exposed ReverseDNS | GuardLogiX</title>
        </MetaTags>
        <Row>
          <Breadcrumbs
            title="Dashboard"
            breadcrumbItem="Reverse DNS Details"
            breadcrumbFeature="Reverse DNS Details"
          />
        </Row>
                          ...
                          <Card>
                            <h5 className="card-header bg-transparent border-bottom  subdomainPortScanCardHead p-0 px-3 pt-1">
                              <div className="mt-4 mt-md-0 d-flex ScanProfileHeading">
                               ExposedReverseDNS - 165112129704189
                              </div>
