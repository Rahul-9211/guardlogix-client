...
const SubdomainFinder = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (cellContent, productData) => (
        <>
          {(() => {
            if (productData.Subdomain === null) {
              return <>-</>;
            } else {
              return <>{productData.Subdomain}</>;
            }
          })()}
        </>
      ),
    ...
    // {
    //   dataField: "Hosting_Provider",
    //   text: "Hosting Provider",
    //   sort: true,
    //   formatter: (cellContent, productData) => (
    //     <>
    //       {(() => {
    //         if (productData.Hosting_Provider === null) {
    //           return <>-</>;
    //         } else {
    //           return  <>{productData.Hosting_Provider}</>;
    //         }
    //       })()}
    //     </>
    //   ),
  ...
  useEffect(async () => {
    ...
    getSubdomainFinderData();
  }, []);

  // ApI to hit Task Data 
  async function getSubdomainFinderData(event) {
    ...
    <React.Fragment>
      <div className="userProfile credentialBreachProfile">
        ...
        <Row>
          <Breadcrumbs
            title="Dashboard"
            breadcrumbItem="Subdomain Details"
            breadcrumbFeature="Subdomain Details"
          />
        </Row>
                        ...
                        <Col lg={12}>
                          <Card>
                            <h5 className="card-header bg-transparent border-bottom  subdomainPortScanCardHead p-0 px-3 pt-1">
                              ...
                              Exposed Credntials - 165112129704687  
                              </div>
